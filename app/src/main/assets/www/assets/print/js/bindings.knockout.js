ko.bindingHandlers.mobileBtnText = {
    update: function(el, va) {
		var val = va();
		if($(el).find('span.ui-btn-text').length > 0){
			$(el).find('span.ui-btn-text').text(ko.utils.unwrapObservable(val));
		} else {
			$(el).text(ko.utils.unwrapObservable(val));
		}
    }
}
