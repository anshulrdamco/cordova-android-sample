...
function jKeyboard($) {
    ...
    var startKb = function () {
        ...
        var kbWidth = $(window).width() * 60.5 / 100;
        var kbScale = kbWidth / 640;
        var kbHeight = 220*kbScale;

        // Setting Keyboard DOM attributes
        $(canvas).attr('id', 'jKeyboard')
		.attr('width', kbWidth)
		.attr('height', kbHeight)
		.attr('style', 'border:none; padding:5px; position: absolute; bottom:0px;\
			color:#fff;\
			float:right;\
			right:0;\
        ...
        kbItems[4] = ['shift', 'z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', '/', 'done'];
        kbItems[5] = ['space'];
        ...
        function replaceIt(txtarea, newtxt) {
            ...
            if (JKEYBOARD == 'search') {
                mainVM.SearchTerm(oldtxt.substr(0, start) +
                  newtxt +
                  oldtxt.substr(end));
            } else if (JKEYBOARD == 'email') {
                mainVM.emailToSendTo(oldtxt.substr(0, start) +
                  newtxt +
                  oldtxt.substr(end));
            } else {
                $(txtarea).val(
        ...
        function paint() {
            roundedRect(ctx, this.x, this.y, this.w, this.h, 5 * kbScale, this.ishover, this.ispress);
            //ctx.strokeRect(this.x , this.y, this.w, this.h ); //ctx.fillText(this.txt, this.x + this.w/2, this.y + this.h/2);
            ctx.drawTextCenter(null, 12 * kbScale, this.x + this.w / 2, this.y + this.h / 2 + 5 * kbScale, this.txt);
        }

        function lookupwidth(c) {
            if (c == 'tab') return 70 * kbScale;
            if (c == 'delete') return 70 * kbScale;
            if (c == 'caps') return 80 * kbScale;
            if (c == 'enter') return 74 * kbScale;
            if (c == 'shift') return 97 * kbScale;
            if (c == 'done') return 102 * kbScale;
            if (c == 'space') return 631 * kbScale;
        ...
        for (var i1 in kbItems) {
            ...
            for (i2 in kbItems[i1]) {
                ...
                var item = {
                    txt: key1, x: kx, y: ky, w: lookupwidth(key1), h: 40 * kbScale,
                    paint: paint,
                    hover: hover,
                    hoveroff: hoveroff,
                    mousedown: lettermousepress,
                    mouseup: mouseup,
                    touchstart: lettermousepress,
                    touchend: mouseup
                };
                kx += item.w + padding;
   ...
   .bind('keydown', '/', function () { kd('/') })
   .bind('keyup', '/', function () { ku('/') })
   .bind('keydown', 'done', function () { kd('done') })
   .bind('keyup', 'done', function () { ku('done') })
   ...
   .bind('keyup', 'space', function () { ku('space') });
        ...
        // To Implement done keys for Language options perhaps?
        keyObjects['done'].mousedown = function () {
            $('#jKeyboard').hide();
        };
        ...
        keyObjects['delete'].mousedown = function () {
            ...
            if ((selection == 0) || af) {
                ...
                if (JKEYBOARD == 'search') {
                    mainVM.SearchTerm(newstr);
                } else if (JKEYBOARD == 'email') {
                    mainVM.emailToSendTo(newstr);
                } else {
                    $(txtarea).val(newstr);
                }
